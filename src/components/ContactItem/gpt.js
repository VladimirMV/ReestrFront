...
import { Container, List, ListItem, ListItemText, ListItemAvatar, Avatar, ListItemSecondaryAction, IconButton, useMediaQuery } from '@mui/material';
import { ContactDescr, ModalPictureWrapper, WrapperBtns } from './ContactItem.styled';
...
const ContactItem = ({
  ...
  return (
    <Container maxWidth="md">
            ...
            <WrapperBtns>
              <ListItemSecondaryAction>
                <Tooltip label="Edit" color="#000" fontSize="xs">
                  <IconButton sx={{ pl: 0 }} edge="end" aria-label="Edit" onClick={() => setModalData()}>
                    <EditIcon />
                  </IconButton>
                </Tooltip>
                <Tooltip label="Delete" color="#000" fontSize="xs">
                  <IconButton sx={{ pl: 2 }} edge="end" aria-label="delete" onClick={() => onDeleteContact(_id)}>
                    <DeleteIcon />
                  </IconButton>
                </Tooltip>
              </ListItemSecondaryAction>
      ...
      <ContactModal isOpen={selectedContact !== null} onClose={closeModal} data={selectedContact} />
    </Container>
  );
};
